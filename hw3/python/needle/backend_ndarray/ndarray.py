...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert all([self.shape[i] == new_shape[i] or self.shape[i] == 1 for i in range(len(self.shape))]), "Invalid broadcast shape"
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        shape = tuple([(idxs[i].stop - idxs[i].start + idxs[i].step - 1) // idxs[i].step for i in range(len(idxs))])
        strides = tuple([self.strides[i] * idxs[i].step for i in range(len(idxs))])
        offset = self._offset + reduce(operator.add, [self.strides[i] * idxs[i].start for i in range(len(idxs))])
