...
__device__ size_t TransformIndex(size_t i, CudaVec shape, CudaVec strides, size_t offset) {
  size_t index = offset;
  for (int j = shape.size - 1; j >= 0; j--) {
    index += (i % shape.data[j]) * strides.data[j];
    i /= shape.data[j];
  }
  return index;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    size_t a_index = TransformIndex(gid, shape, strides, offset);
    out[gid] = a[a_index];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                                   CudaVec strides, size_t offset) {
  /**
   * Set items in a (non-compact) array using CUDA.  This is the kernel that will do the actual work.
   * 
   * Args:
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[TransformIndex(gid, shape, strides, offset)] = a[gid];
  }
  /// END SOLUTION
}
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[TransformIndex(gid, shape, strides, offset)] = val;
  }
  /// END SOLUTION
}
...
void ScalarMul(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarOpKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size, Mul());
}
...
void ScalarDiv(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarOpKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size, Div());
}
...
void ScalarEq(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarOpKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size, Eq());
}
...
void ScalarGe(const CudaArray& a, scalar_t val, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  ScalarOpKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size, Ge());
}
